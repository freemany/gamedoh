...
Rule of thumb: if a number appears in game logic, it belongs in `constants.js` with a descriptive name. This includes **all timeouts and delays** — `setTimeout(..., 1500)` in game logic is a bug waiting to happen. Name it:

```js
export const STAGE_CLEAR_DELAY  = 1500; // ms pause after last enemy dies before advancing
export const RESPAWN_DELAY      = 1000; // ms before player respawns after death
export const LEVEL_INTRO_DURATION = 2000; // ms the "STAGE X / GET READY!" screen shows
```

Then reference them:
```js
setTimeout(() => this.level.advance(), STAGE_CLEAR_DELAY);
```
...
## Level Transitions — Common Mistakes

Three bugs appear repeatedly across games that use level intros and countdowns. Fix all three proactively.

### 1. Countdown fires at game start (should only appear between levels)

**Symptom:** The 3-2-1 countdown shows before the very first level, which feels wrong — the player hasn't done anything yet.

**Cause:** `_startLevel()` unconditionally calls the countdown.

**Fix:** Gate the countdown on `level.index > 0`:
```js
async _startLevel() {
  this._transitioning = true;
  await showLevelIntro(this, this.level.index + 1);
  if (this.level.index > 0) await showCountdown(this); // ← skip on first level
...
### 2. Level intro and countdown are invisible (black text on black background)

**Symptom:** The transition appears to happen (game pauses briefly) but nothing is visible on screen.

**Cause:** The engine's built-in `LevelIntro` and `Countdown` classes draw with `fillStyle = 'Black'` and `strokeStyle = 'Black'` — invisible against the game's black background.

**Fix:** Never use the engine's `LevelIntro` or `Countdown` directly. Write custom `showLevelIntro` and `showCountdown` functions in `screens.js` that use light colours and a semi-transparent dark overlay:

```js
import { Timer } from './gamedoh-engine.js';
import { CANVAS_WIDTH, CANVAS_HEIGHT } from './constants.js';

export async function showLevelIntro(game, levelNum) {
  return new Promise((resolve) => {
    game.transitionRenderer = () => {
      ...
      ctx.fillStyle = '#ffff00';               // ← visible yellow, not black
    ...
    setTimeout(() => { game.transitionRenderer = null; resolve(); }, 2000);
  });
}
...
export async function showCountdown(game) {
  await new Timer(3).start((remaining) => {
    game.transitionRenderer = () => {
      ...
      ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
      const colour = remaining === 1 ? '#ff4444' : remaining === 2 ? '#ff9800' : '#00ccff';
      ctx.beginPath();
      ctx.arc(cx, cy, 60, 0, Math.PI * 2);
      ctx.strokeStyle = colour;
      ctx.lineWidth = 4;
      ctx.stroke();
      ctx.fillStyle = colour;                  // ← visible colour, not black
...
Then in `game.js`, import and use these instead of the engine classes:
```js
import { showLevelIntro, showCountdown } from './screens.js';
// Never: import { LevelIntro, Countdown } from './gamedoh-engine.js';
```

---

### 3. Levels skip (stage 1 → stage 3, bypassing stage 2)

**Symptom:** After clearing a stage, the game jumps forward by two levels instead of one.

**Cause:** `level.advance()` increments `index` and sets `_pendingLevelStart = true`, but never calls `onLevelAdvance()`. Without `onLevelAdvance()` → `setupLevel()`, the enemy/entity arrays are never rebuilt. On the very next frame after the transition, `_update()` sees `enemies.length === 0` again and fires `level.advance()` a second time — skipping a level.

**Fix:** `level.advance()` must call `onLevelAdvance()` (which calls `setupLevel()` in `main.js`). `setupLevel()` already sets `_pendingLevelStart = true`, so remove it from `advance()`:

```js
// level.js
advance() {
  if (this.isLast()) { this.game.win(); return; }
  this.index++;
  if (this.game.onLevelAdvance) this.game.onLevelAdvance(); // ← rebuilds entities + sets _pendingLevelStart
  // Never set _pendingLevelStart = true here directly — setupLevel() does it
}

// main.js
const setupLevel = () => {
  game.enemies = buildEnemies(game);   // must create entities so enemies.length > 0 immediately
  // ...
  game._pendingLevelStart = true;      // triggers _startLevel() on next frame
};
game.onLevelAdvance = () => setupLevel();
```

The win-condition check in `_update()` must also be guarded so it does not fire during the 1-frame gap between `setupLevel()` and `_startLevel()`:
```js
// In _update(): only advance when not already transitioning
if (this.enemies.length === 0 && !this._transitioning) {
  this._transitioning = true;          // ← set immediately to block re-entry
  setTimeout(() => { this.level.advance(); }, 1500);
}
```
...
## Keeping Class Size Small — Extract to `screens.js`

Game classes (`game.js`) should contain only game logic: state management, entity updates, collision, spawning, and lifecycle hooks. **Drawing implementations do not belong in the class.**

If a draw method grows beyond a few lines, extract it to a standalone function in `screens.js` and call it from the class:

```js
// screens.js — standalone functions, no class dependency
export const drawHUD = (game) => { /* full implementation */ };
export const drawWorld = (game) => { /* full implementation */ };

// game.js — thin delegates only
drawHUD()    { drawHUD(this); }
_drawWorld() { drawWorld(this); }
```

**Rule of thumb:** if a method in `game.js` is longer than ~10 lines, it should live in `screens.js`.

The `screens.js` file owns all drawing functions:
- `drawStartScreen(ctx, startButton)`
- `drawPauseOverlay(ctx)`
- `drawGameOverScreen(ctx, score, restartButton)`
- `drawWinScreen(ctx, score, restartButton)`
- `drawHUD(game)` — pass the whole game object when many properties are needed
- `drawWorld(game)` — grid, entities, overlays
